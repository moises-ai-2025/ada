...
import { Heart, Notebook } from "@phosphor-icons/react/dist/ssr"
import { HistoryItem } from "./HistoryItem"
import { DialogRoot, DialogButton, Dialog, DialogTitle, DialogDescription } from "../ui/OldDialog"
import { binDates } from "./date-binning"
import { providerStore } from "@/lib/stores/provider" // Ajuste o caminho conforme necessário
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const currentProvider = useStore(providerStore); // Obtém o provedor atual
          ...
          <div className="!bg-background !hover:bg-background !active:bg-background flex items-center h-full w-full justify-between max-w-full">
            ...
            <DialogRoot open={dialogContent !== null}>
              ...
              <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
                {dialogContent && dialogContent.type === 'delete' && (
                  <>
        ...
        items: [
          <div key="models-container" className="!bg-background !hover:bg-background active:bg-background flex flex-col !w-full h-full items-center justify-center">
            <div className="text-xs text-muted text-start">
              Current Model: {currentProvider.model.displayName}
            </div>
            {/* Removido o mapeamento de modelos, pois os arrays estão vazios */}
          </div>,
        ],
      },
      {
