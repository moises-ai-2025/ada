...
var auth = {
  init({ editorOrigin: editorOrigin2, clientId, scope, forwardAuth, redirectUri }) {
    ...
    if (editorOrigin2) {
      authState.editorOrigin = new URL(editorOrigin2).origin;
    }
    ...
    authState.cleanup.push(() => {
      var _a;
      return (_a = authState.broadcastChannel) == null ? void 0 : _a.removeEventListener("message", onChannelMessage);
    });
    if (authState.tokens) {
    ...
    if (searchParams.has(SEARCH_PARAM_AUTH_CODE)) {
      ...
      redirectUri2 = authState.redirectUri;
      Tokens.fromAuthCode({
  ...
  forget() {
    var _a;
    (_a = authState.tokens) == null ? void 0 : _a.forget();
...
var _WebContainer = class _WebContainer {
  ...
  mount(snapshotOrTree, options) {
    const payload = snapshotOrTree instanceof Uint8Array ? snapshotOrTree : snapshotOrTree instanceof ArrayBuffer ? new Uint8Array(snapshotOrTree) : encoder.encode(JSON.stringify(formatFileSystemTree(snapshotOrTree)));
...
var WebContainerInternal = class {
  ...
  serialize(folder, options) {
    return this._instance.serialize(folder, options);
  }
};
...
function getIframeUrl() {
  ...
  url.searchParams.set("version", "1.3.0-internal.10");
  return url;
}
